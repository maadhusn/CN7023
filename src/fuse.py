...
def get_ann_predictions(
    classifier: HOGClassifier, features: np.ndarray, labels: np.ndarray
) -> tuple:
...
class EnsembleFusion:
    ...
    def fit(self, cnn_probs: np.ndarray, ann_probs: np.ndarray, y_true: np.ndarray):
        ...
        if self.fusion_method == "learned_weights":
            ...
            print(
                f"Learned weights - CNN: {self.cnn_weight:.3f}, ANN: {self.ann_weight:.3f}"
            )
            print(f"Validation accuracy with learned weights: {best_accuracy:.4f}")

        self.is_fitted = True

    def predict(self, cnn_probs: np.ndarray, ann_probs: np.ndarray) -> tuple:
        ...
        if (
            ...
            or self.fusion_method == "learned_weights"
        ):
...
def evaluate_fusion_methods(
    cnn_probs: np.ndarray, ann_probs: np.ndarray, y_true: np.ndarray, class_names: list
) -> dict:
    ...
    for method in fusion_methods:
        ...
        results[method] = {
            ...
            "fusion_weights": {"cnn": fusion.cnn_weight, "ann": fusion.ann_weight},
        }
...
def load_mlp_model(device: torch.device) -> torch.nn.Module:
    ...
    with open("report_assets/ann_metrics.json") as f:
...
def get_mlp_predictions(
    ...
    dataset = torch.utils.data.TensorDataset(
        torch.FloatTensor(features_scaled), torch.LongTensor(labels)
    )
...
def main():
    ...
    num_classes = len(
        config.get("classes", ["class_0", "class_1", "class_2", "class_3"])
    )
    ...
    print(
        f"{'Fusion':<15} {fused_test_acc:<10.4f} {fused_test_acc - max(ann_test_acc, cnn_test_acc):+.4f}"
    )
    print("=" * 60)
