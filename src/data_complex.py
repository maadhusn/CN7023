    ...
    def _create_synthetic_data(self) -> Tuple[List[Tuple[str, int]], List[str]]:
        ...
        for class_idx, class_name in enumerate(classes):
            for i in range(samples_per_class):
                synthetic_path = (
                    f"synthetic/{self.variant}/{class_name}/img_{i:03d}.jpg"
                )
        ...
        if not manifest_path.exists():
            ...
            if classes:
                ...
                raise FileNotFoundError(
                    f"Manifest not found and no classes discovered: {manifest_path}"
                )
...
def create_manifests(
    ...
    if not regen_splits and all(
        m.exists() for m in [train_manifest, val_manifest, test_manifest]
    ):
    ...
    print(
        f"Created manifests: {len(train_samples)} train, {len(val_samples)} val, {len(test_samples)} test"
    )


def create_eda_visualizations(
    root: Path, variant: str, save_dir: str = "report_assets"
):
    ...
    for bar, count in zip(bars, class_counts.values()):
        ...
        ax.text(
            ...
            height + 0.01 * max(class_counts.values()),
            f"{count}",
            ha="center",
            va="bottom",
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "class_counts.png"), dpi=150, bbox_inches="tight"
    )
    plt.close()
    ...
    if sample_images:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(save_dir, "samples_grid.png"), dpi=150, bbox_inches="tight"
        )
        plt.close()
...
def get_transforms(config: Dict, is_training: bool = True) -> transforms.Compose:
    ...
    if is_training:
        ...
        if aug_config.get("rotation_deg", 0) > 0:
            transform_list.append(transforms.RandomRotation(aug_config["rotation_deg"]))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Data processing with path-preserving policies"
    )
    parser.add_argument(
        "--regen-splits",
        action="store_true",
